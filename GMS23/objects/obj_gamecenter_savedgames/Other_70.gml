...
switch(async_load[?"type"])
{
	...
	case "GameCenter_SavedGames_Fetch":
		...
		var slotsArray = json_parse(async_load[?"slots"]);	
		var count = array_length(slotsArray)
		for(var i = 0 ; i < count ; i++)
		{
			var slotData = slotsArray[i];
	...
	case "GameCenter_SavedGames_GetData":
		...
		var count = array_length(dataArray);
		for(var i = 0 ; i < count; i++)
		{
	...
	case "GameCenter_SavedGames_ResolveConflict":
		...
		var conflict_ind = async_load[?"conflict_ind"]
		// Early exit if not successful
		if (!async_load[?"success"]) break;		
		break;
	...
	case "GameCenter_SavedGames_HasConflict":
		...
		// GameCenter_SavedGames_ResolveConflict(conflict_ind, "<the data that you choose>")
